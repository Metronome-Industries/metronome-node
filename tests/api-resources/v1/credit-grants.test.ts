...
describe('resource creditGrants', () => {
  test('create: only required params', async () => {
    const responsePromise = client.v1.creditGrants.create({
  ...
  test('create: required and optional params', async () => {
    const response = await client.v1.creditGrants.create({
      customer_id: '9b85c1c1-5238-4f2a-a409-61412905e1e1',
      expires_at: '2022-04-01T00:00:00Z',
      grant_amount: { amount: 1000, credit_type_id: '5ae401dc-a648-4b49-9ac3-391bb5bc4d7b' },
      name: 'Acme Corp Promotional Credit Grant',
  ...
  test('list: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.v1.creditGrants.list({ path: '/_stainless_unknown_path' })).rejects.toThrow(
  ...
  test('listEntries: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.v1.creditGrants.listEntries({ path: '/_stainless_unknown_path' })).rejects.toThrow(
      Metronome.NotFoundError,
    );
  });
