...
describe('resource plans', () => {
  ...
  test('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.v1.plans.list({ limit: 1, next_page: 'next_page' }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('listCustomers: only required params', async () => {
    const responsePromise = client.v1.plans.listCustomers({ plan_id: 'd7abd0cd-4ae9-4db7-8676-e986a4ebd8dc' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
