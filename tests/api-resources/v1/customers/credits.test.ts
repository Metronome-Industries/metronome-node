...
describe('resource credits', () => {
  ...
  test('list: only required params', async () => {
    const responsePromise = client.v1.customers.credits.list({ customer_id: '13117714-3f05-48e5-a6e9-a66093f13b4d' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test('updateEndDate: only required params', async () => {
    const responsePromise = client.v1.customers.credits.updateEndDate({
