...
import { APIPromise } from '@metronome/sdk/core/api-promise';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['METRONOME_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Metronome) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Metronome({ logger: logger, logLevel: 'debug', bearerToken: 'My Bearer Token' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Metronome({ logger: logger, logLevel: 'info', bearerToken: 'My Bearer Token' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['METRONOME_LOG'] = 'debug';
      const client = new Metronome({ logger: logger, bearerToken: 'My Bearer Token' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['METRONOME_LOG'] = 'not a log level';
      const client = new Metronome({ logger: logger, bearerToken: 'My Bearer Token' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'METRONOME_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['METRONOME_LOG'] = 'debug';
      const client = new Metronome({ logger: logger, logLevel: 'off', bearerToken: 'My Bearer Token' });
  ...
  describe('baseUrl', () => {
    ...
    test('explicit option', () => {
      const client = new Metronome({ baseURL: 'https://example.com', bearerToken: 'My Bearer Token' });
    ...
    test('in request options overridden by env variable', () => {
      process.env['METRONOME_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Metronome({ bearerToken: 'My Bearer Token' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Metronome({ maxRetries: 4, bearerToken: 'My Bearer Token' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Metronome({ bearerToken: 'My Bearer Token' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['METRONOME_BEARER_TOKEN'] = 'My Bearer Token';
    const client = new Metronome();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['METRONOME_BEARER_TOKEN'] = 'another My Bearer Token';
    const client = new Metronome({ bearerToken: 'My Bearer Token' });
    expect(client.bearerToken).toBe('My Bearer Token');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Metronome({ bearerToken: 'My Bearer Token', timeout: 10, fetch: testFetch });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Metronome({ bearerToken: 'My Bearer Token', fetch: testFetch });
