...
```sh
git clone git@github.com:stainless-sdks/metronome-typescript.git
cd metronome-typescript
./scripts/bootstrap
./scripts/build
```
...
> [!NOTE]
> Once this package is [published to npm](https://www.stainless.com/docs/guides/publish), this will become: `npx -y @metronome/mcp`
...
```json
{
  "mcpServers": {
    "metronome_sdk_api": {
      "command": "node",
      "args": [
        "/path/to/local/metronome-typescript/packages/mcp-server",
        "--client=claude",
        "--tools=dynamic"
      ],
      "env": {
